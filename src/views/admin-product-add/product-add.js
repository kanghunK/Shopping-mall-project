...
async function getSizeOfCategory(){
    ...
    const categoryInfo = await Api.get('/api/admin/product/category/size/'+ categoryName);
...
async function handleSubmit(e) {
    ...
    if (check){
        try{
            ...
            const data = {category, brand, productName, price, launchDate, img, quantity, size};

            const result = await Api.post('/api/admin/product', data);

            if (result){
                console.log(result);
                alert(`${result.productName} 상품이 성공적으로 등록되었습니다!`);
            }
